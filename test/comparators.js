...
async function testIsZeroDL(input, circuit) {
  let real_result = [0n];
  if (input == 0n) {
    real_result = [1n];
  }
...
describe("IsZeroDL test", function () {
  this.timeout(100000);
  let circuit;

  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "bitify", "isZero.circom")
    );
  });

  it("0 ?= 0", async function () {
    await testIsZeroDL(0n, circuit);
  });

  it("1 ?= 0", async function () {
    await testIsZeroDL(1n, circuit);
  });
});

describe("IsEqualDL test", function () {
  this.timeout(100000);
  let circuit;

  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "bitify", "isEqual.circom")
    );
  });

  it("0 ?= 0", async function () {
    await testIsEqualDL(0n, 0n, circuit);
  });

  it("1 ?= 0", async function () {
    await testIsEqualDL(1n, 0n, circuit);
  });

  it("512 ?= 234", async function () {
    await testIsEqualDL(512n, 234n, circuit);
  });

  it("512 ?= 512", async function () {
    await testIsEqualDL(512n, 512n, circuit);
  });
});
...
describe("IsGreater test", function () {
  ...
  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "bitify", "greaterThan.circom")
    );
  });
...
describe("IsGreaterEq test", function () {
  ...
  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "bitify", "greaterEqThan.circom")
    );
  });
...
describe("IsLess test", function () {
  ...
  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "bitify", "lessThan.circom")
    );
  });
...
describe("IsLessEq test", function () {
  ...
  before(async () => {
    circuit = await wasm_tester(
      path.join(__dirname, "circuits", "bitify", "lessEqThan.circom")
    );
  });
